...
export const perpsDepositAction: Action = {
  ...
  handler: async (_runtime, message, _state, _options, callback): Promise<ActionResult> => {
    ...
    await callback?.({ text: `Depositing $${amount} USDC to pacific...` } as Parameters<HandlerCallback>[0]);
    ...
    const resultText = `Deposited $${amount} USDC to pacific. TX: ${sig}`;
  ...
  examples: [
    [
      { name: '{{user1}}', content: { text: 'Deposit 50 USDC to pacific' } },
      { name: 'lpcli', content: { text: 'Depositing $50 USDC to your pacific account.' } },
    ],
  ],
};
